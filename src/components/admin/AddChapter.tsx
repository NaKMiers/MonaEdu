...
function AddChapter({
  ...
  useEffect(() => {
    setValue(
      "order",
      chapters.reduce((max, chapter) => Math.max(max, chapter.order), 0) + 1
    );
  }, [chapters, setValue]);
  ...
  const handleUpdateChapter: SubmitHandler<FieldValues> = useCallback(
    async (data) => {
      ...
      try {
        const { updatedChapter, message } = await updateChapterApi(
          editingValues._id,
          data
        );
  ...
  return (
    <div
      className={`bg-white px-3 py-2 rounded-lg flex items-start gap-2 ${className}`}
    >
      {/* Title */}
      <Input
        id="title"
          ...
          <button
            ...
            onClick={() =>
              setEditingValues({
                ...
                order:
                  chapters.reduce(
                    (max, chapter) => Math.max(max, chapter.order),
                    0
                  ) + 1,
              })
            }
        ...
        <button
          ...
          onClick={
            !editingValues?._id
              ...
              : handleSubmit(handleUpdateChapter)
          }
        >
          {loading ? (
            <RiDonutChartFill
