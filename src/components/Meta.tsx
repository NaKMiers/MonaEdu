...
function Meta({
  ...
  >(
    []
      .concat(
        (searchParams?.ctg || categories.map((category) => category.slug)) as []
      )
      .map((type) => type)
  );
  // states
  const [selectedFilterTags, setSelectedFilterTags] = useState<string[]>(
    []
      .concat((searchParams?.tag || tags.map((tag) => tag.slug)) as [])
      .map((type) => type)
  );
  ...
  const handleOptimizeFilter: SubmitHandler<FieldValues> = useCallback(
    ({ data, newSelectedCates, newSelectedTgs }) => {
      ...
      return {
        ...
        search: search || "",
        ctg:
          newSelectedCates.length === categories.length ? [] : newSelectedCates,
  ...
  const handleFilter: SubmitHandler<FieldValues> = useCallback(
    async ({ data, newSelectedCates, newSelectedTgs }) => {
      const params: any = handleOptimizeFilter({
        data,
        newSelectedCates,
        newSelectedTgs,
      });
  ...
  return (
    <div
      className={`self-end w-full text-dark trans-300 no-scrollbar p-21 ${className}`}
    >
      {/* MARK: Filter */}
