...
function AllLessons() {
  ...
  useEffect(() => {
    const lessons: ILesson[] = chapters
      .map((chapter) => chapter.lessons)
      .flat() as ILesson[];
    const curLessonIndex = lessons.findIndex(
      (lesson) => lesson.slug === lessonSlug
    );

    setPrevLesson(curLessonIndex > 0 ? lessons[curLessonIndex - 1].slug : "");
    setNextLesson(
      curLessonIndex < lessons.length - 1
        ? lessons[curLessonIndex + 1].slug
        : ""
    );
  }, [chapters, lessonSlug]);
  ...
  return (
    <>
      ...
      <div
        className={`${
          openSidebar ? "sm:max-w-[300px]" : "sm:max-w-0"
        } sm:w-full trans-300`}
      />
            ...
            <div className="relative overflow-hidden rounded-md w-full h-6 shadow-sm shadow-primary">
              ...
              <div className="absolute flex items-center justify-between top-1/2 px-4 left-0 right-0 -translate-y-1/2 text-orange-400 text-sm font-body tracking-wider font-semibold drop-shadow-sm">
                ...
                <span>
                  10/
                  {chapters.reduce(
                    (acc, chapter) => acc + chapter.lessonQuantity,
                    0
                  )}
                </span>
              </div>
