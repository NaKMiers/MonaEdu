...
export async function POST(
  ...
  try {
    ...
    if (
      (voucher.owner as IUser).email === email ||
      (voucher.owner as IUser).email === userEmail
    ) {
      return NextResponse.json(
    ...
    if (voucher.usedUsers.includes(email || userEmail)) {
      return NextResponse.json(
        {
          message:
            "Bạn đã dùng mã khuyến mãi này rồi, vui lòng thử một mã khác!",
        },
        { status: 401 }
      );
    }
    ...
    if (voucher.expire && new Date() > new Date(voucher.expire)) {
      return NextResponse.json(
        { message: "Mã khuyến mãi của bạn đã hết hạn" },
        { status: 401 }
      );
    }
    ...
    if ((voucher.timesLeft || 0) <= 0) {
      return NextResponse.json(
        { message: "Mã khuyến mãi của bạn đã hết lượt dùng" },
        { status: 401 }
      );
    }
    ...
    if (total < voucher.minTotal) {
      return NextResponse.json(
        {
          message: `Chỉ áp dụng đối với đơn hàng tối thiểu ${formatPrice(
            voucher.minTotal
          )}`,
        },
        { status: 401 }
    ...
    switch (voucher.type) {
      case "fixed-reduce": {
        message = `Bạn được giảm ${formatPrice(
          Math.abs(+voucher.value)
        )} từ tổng giá trị đơn hàng`;
        break;
      }
      case "fixed": {
        message = `Đơn hàng của bạn sẽ có giá là ${formatPrice(
          +voucher.value
        )}`;
        break;
      }
      case "percentage": {
        message = `Bạn được giảm ${voucher.value}, tối đa ${Intl.NumberFormat(
          "en-US",
          {
            style: "currency",
            currency: "USD",
